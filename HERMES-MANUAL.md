...
> **Note for the Human Operator:** For installation, environment setup, model selection, and step-by-step user guides, please refer to [Guide.md](Guide.md).

---

## Touchpoint Protocol — When Hermes Must Stop and Await Human Input

Hermes operates autonomously. Hermes must pause execution and await user response **only** at the following designated touchpoints:

| # | Step | When Hermes Stops | What Hermes Waits For |
|---|---|---|---|
| 1 | **Step 3** | When `plan.md` and `task.md` drafting and quality self-audit are complete | User review and explicit reply *"approved"* (or applied revisions if requested). |
| 2 | **Step 3.1** | Prior to GitHub repository setup | Target repo name (`<owner>/<repo>`) and confirmation that `GITHUB_TOKEN` is exported. |
| 3 | **Step 3.2** | Prior to writing core business code | User answers to the single structured list of domain/operational questions. |
| 4 | **Step 4** | If a Loop Engineering build/test/fix cycle reaches 5 failed iterations | Specific blocker report presented; wait for user guidance. |
| 5 | **Step 5** | When a semantic merge conflict cannot be auto-resolved | Both conflicting versions presented; wait for user decision on winning version. |
| 6 | **Step 9** | When a tech-stack or architectural change alters `plan.md` Sections 5–8 | Diff presented; wait for user re-approval of that specific section. |
| 7 | **Step 10.A** | When `SYSTEM-PLAN.md` is drafted (microservices projects) | User review and explicit approval of `SYSTEM-PLAN.md` before any per-service plan is generated. |
| 8 | **Step 10.C** | When an inter-service API contract modification is required | `SYSTEM-PLAN.md` diff presented; wait for user approval before modifying service code. |
...
| Step 4 — Implementation, phase by phase | `phase-implementation` + `loop-engineering` + `backend-development` (for any server-side code) + `frontend-development` (for any UI/client-side code) | `skills/phase-implementation/SKILL.md` + `skills/loop-engineering/SKILL.md` + `skills/backend-development/SKILL.md` + `skills/frontend-development/SKILL.md` |
...
3. **Confirm mandatory skills are available.** Check that all 17 skills in `skills/` are loadable:
   - `plan-generation`, `task-generation`, `human-review-gate`, `github-setup`, `domain-clarification`, `phase-implementation`, `loop-engineering`, `multi-dev-merge`, `production-documentation`, `git-github-automation`, `deployment-verification`, `feature-tech-stack-change`, `microservices-adaptation`, `quality-checklist`, `context-compression`, `backend-development`, `frontend-development`.
   - Note: the first 14 are generated by `HERMES-SKILL-BUILDER.md`; `context-compression`, `backend-development`, and `frontend-development` are maintained directly in `skills/` and are **not** produced by the builder — re-running it will not recreate them.
   - `frontend-development` also ships with a companion manual: `skills/frontend-development/FRONTEND-MANUAL.md` — covering usage instructions, the 12-step workflow overview, and a guide to the reference material in `skills/frontend-development/references/`.
   - If any named skill is missing, stop and tell the human which one is missing (and, for the first 14, that `HERMES-SKILL-BUILDER.md` needs to be (re)run) — do not attempt to reconstruct a missing skill's content from memory.
...
**Whenever a phase involves backend/server-side code (APIs, DB, services, auth, background jobs), keep `skills/backend-development/SKILL.md` open and follow it — it is the binding quality bar for all backend code, applied on top of the Ponytail minimal-diff ladder below.**
**Whenever a phase involves frontend/UI code (pages, components, state, forms, styling, client-side routing), keep `skills/frontend-development/SKILL.md` open and follow it — it is the binding quality bar for all frontend code, applied on top of the Ponytail minimal-diff ladder below.**
